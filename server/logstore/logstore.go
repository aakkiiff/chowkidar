...
// PruneOrphans walks LOG_DIR and removes any agent subdirectory whose id is
// not in validIDs. Covers the small window where an agent's in-flight log
// POST resurrects its log dir between DB deletion and token rejection.
func (s *Store) PruneOrphans(validIDs map[string]struct{}) {
	...
	for _, e := range entries {
		...
		if _, ok := validIDs[e.Name()]; ok {
			continue
		}
		_ = s.DeleteAgent(e.Name())
	}
}

// DeleteAgent closes any open writers for the given agent and removes its
// entire log directory (active file + rotations).
func (s *Store) DeleteAgent(agentID string) error {
	...
	s.mu.Unlock()
	return os.RemoveAll(filepath.Join(s.cfg.Dir, agentID))
}
...
// tailLines reads up to n trailing lines from f. Starts with a small 128 KB
// window and doubles on each pass until n newlines are counted or the file
// start is reached. Keeps RSS tiny for typical tails (a hundred lines ≈ 30 KB)
// while still handling long lines or sparse newline files up to a hard cap.
func tailLines(f *os.File, n int) [][]byte {
	...
	for {
		...
		// Enough newlines, or we've already read the whole file, or hit the
		// hard cap — stop growing and slice the result.
		if nl > n || window == size || window == hardCap {
			all := bytes.Split(bytes.TrimRight(buf, "\n"), []byte("\n"))
			if len(all) > n {
				all = all[len(all)-n:]
			}
